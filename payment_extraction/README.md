...
| **Extracted text records** | **2.87M pages** | 993,406 pages | — | — |
...
| **Payment records extracted** | **10,118** across 10 types ($821M) | 0 | 0 | 0 |
...
| **Extraction Phases** | 24 + Phase 5I payment sweep |
| **Contamination Bugs Caught & Fixed** | 9 |
| **Wire Transfers in Master Ledger** | 382 |
| **Payment Records Extracted (Phase 5I)** | 10,118 across 10 types ($821M) |
| **Shell-to-Shell Transfers Identified** | 43 |
| **Shell Trust Hierarchy Tiers Mapped** | 4 |

### Phase 5I: Unified Payment Extraction

In addition to the 382-wire master ledger from Phases 14-24, a unified payment extraction pipeline parsed 4,888 JPMorgan Transaction Detail pages and 37 SWIFT MT103 pages, extracting **10,118 payment records** totaling **$821,246,319** across 10 payment types:

| Payment Type | Count | Volume (Unverified) |
|---|---|---|
| Book Transfer | 940 | $182,477,867 |
| Fedwire | 3,766 | $166,769,169 |
| Foreign Remittance | 470 | $147,465,696 |
| Internal Transfer | 107 | $99,212,559 |
| CHIPS | 327 | $65,788,493 |
| SWIFT MT103 | 22 | $50,298,931 |
| Check | 4,122 | $49,069,830 |
| Online Transfer | 84 | $46,105,370 |
| ACH | 176 | $7,629,241 |
| Chase Card | 104 | $4,644,090 |
| **Total** | **10,118** | **$819,461,247** |

**Data quality:** 72.9% with extracted amount, 93.9% with date, 47.4% with beneficiary name, 69.0% with running balance. The 27% amount gap is structural — OCR linearizes columnar check entries, losing column alignment for short single-line transactions. High-value wire types (Fedwire, CHIPS, SWIFT, Book Transfer) have substantially higher amount coverage.

**Entity coverage:** 1,055 unique entities identified, including LSJ LLC ($35.1M across 399 transactions), Ghislaine Maxwell ($12.0M across 15 transactions), The Haze Trust ($60.0M), Sikorsky Aircraft ($10.4M), Bear Stearns Securities ($7.1M), and 104 Merchants Commercial Bank transactions ($4.2M).
...
## Database Schema (36 Tables, 26.6M Rows)
...
* `fund_flows` — 23,832 directional money movements (entity\_from → entity\_to, amount, date, confidence)
* `fund_flows_audited` — 7,355 classified financial flows (5-tier: PROVEN/STRONG/MODERATE/WEAK/VERY\_WEAK)
* `verified_wires` — 185 court-exhibit authenticated wire transfers (dates, bates numbers, exhibits)
* `extracted_payments` — 10,118 payment records across 10 types ($821M) from JPM Transaction Detail + SWIFT MT103 pages
...
* `entity_registry` — Canonical entity names with fuzzy-match resolution from payment extraction
* `entity_aliases` — Known alias mappings for target entities
...
* `extracted_text` — 2,866,239 OCR text pages
* `dates_found` — Temporal mapping across entire corpus
* `media_evidence` — DS10 image/video catalog (503K images + 874 videos)
* `statement_chain` — Balance continuity tracking across JPM statement pages

**External Cross-Reference**

* `faa_master`, `faa_engine`, `faa_acftref`, `faa_dereg`, `faa_dealer`, `faa_docindex`, `faa_reserved` — FAA aircraft registry for flight tracking
* `icij_entities`, `icij_officers`, `icij_relationships`, `icij_addresses`, `icij_intermediaries`, `icij_others` — ICIJ Offshore Leaks for shell company cross-referencing
...
Phase 5I   Payment Extraction Pipeline → 10,118 records, $821M, 10 payment types
...
### Payment Extraction Pipeline (Phase 5I)

| Step | What Happened | Impact |
|------|--------------|--------|
| Scope scan | Identified 7,653 JPM Transaction Detail pages + 37 MT103 pages | Defined extraction target |
| Schema discovery | Mapped actual DB schema (extracted_text JOIN files) | Fixed column name assumptions |
| Architecture pivot | Replaced 6 separate extractors with unified JPM parser | One parser, all payment types |
| JPM parser v3 | 4,888 pages parsed, 10,203 records, 9 type classifiers | $771M from JPM statements |
| MT103 extractor | 22 strict SWIFT records from 53 candidates | $50M additional |
| Entity resolution | 1,055 unique entities, fuzzy matching, FX clearing blocklist | 47.4% beneficiary coverage |
| Transaction linking | 183 linked records: 10 reversal pairs, 101 cross-type, 72 adjacent-day | Dedup + relationship mapping |
| DB insertion | 10,118 new records into `extracted_payments` | Live in production DB |
...
- **PROVEN** (≥12): Bank statement language, multi-axis confirmation, ctx\_txn ≥ 2
- **STRONG** (8-11): Good signals, minor gaps
- **MODERATE** (5-7): Mixed signals
- **WEAK** / **VERY\_WEAK** / **REJECT**: Insufficient evidence or known noise
...
|-----------|-----------|--------|
| **WEAK/VERY\_WEAK tier exclusions** | **Yes — $5M-$15M** | $991M excluded as low-confidence; manual review of top entries could recover $5-15M |
| **Payment extraction amount gaps** | **Partial — 2,668 records** | 27.1% of extracted payments lack amounts due to OCR column alignment loss on short check entries |
...
Only one gap ($5-15M excluded tiers) has a credible dollar estimate. The payment extraction amount gap is structural (OCR column loss) and recoverable via layout-aware PDF re-extraction — a future phase. The others are real information gaps with unknown values. I am not going to put specific ranges on things I cannot measure.
...
├── METHODOLOGY.md                         ← 24-phase pipeline + Phase 5I, 9 bugs, 5-axis scoring, limitations
...
- **[SCHEMA.md](SCHEMA.md)** — Full database architecture showing how 36 tables, 11.4M entities, and 1.48M files feed into the 382-wire master ledger and 10,118 payment records
...
- **Chain of custody**: The 8.0GB forensic database represents a consolidated analytical environment. Releasing it in fragments could enable miscontextualization of intermediate results without the full pipeline logic that produced them.
...
| Feb 24 | Payment extraction pipeline (Phase 5I): 10,118 records, $821M, 10 payment types from 4,888 JPM pages |
